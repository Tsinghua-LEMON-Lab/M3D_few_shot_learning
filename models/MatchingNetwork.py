...
class MatchingNetwork(nn.Module):
    def __init__(self, keep_prob, k_bits,
                 batch_size=100, num_channels=1, learning_rate=0.001, fce=False, num_classes_per_set=5,
                 num_samples_per_class=1, nClasses = 0, image_size = 28, test_only=False):
        ...
        self.fce = fce
        self.g = Classifier(layer_size = 64, k_bits=k_bits, num_channels=num_channels,
                            nClasses= nClasses, image_size = image_size ).to(device)
        # self.g = Classifier(layer_size=64, k_bits=k_bits, num_channels=num_channels,
        #                     nClasses=nClasses, image_size=image_size)
        if fce:
            self.lstm = BidirectionalLSTM(layer_sizes=[32], batch_size=self.batch_size, vector_dim = self.g.outSize).to(device)
            # self.lstm = BidirectionalLSTM(layer_sizes=[32], batch_size=self.batch_size, vector_dim=self.g.outSize)
        self.dn = DistanceNetwork().to(device)
        # self.dn = DistanceNetwork()
        self.classify = AttentionalClassify().to(device)
    ...
    def forward(self, support_set_images, support_set_labels_one_hot, target_image, target_label):
        ...
        for i in np.arange(target_image.size(1)):
            ...
            similarities = self.dn(support_set=outputs[:-1], input_image=outputs[-1], k_bits=self.k_bits, test_only=self.test_only)
