...
class OneShotBuilder:
    ...
    def __init__(self, data,LOG_DIR, SAVE_DIR):
        """
    ...
    def build_experiment(self, batch_size, k_bits, classes_per_set, samples_per_class, channels, fce, test_only):
        ...
        self.matchingNet = MatchingNetwork(batch_size=batch_size, k_bits=k_bits,
                                         keep_prob=self.keep_prob, num_channels=channels,
                                         fce=fce,
                                         num_classes_per_set=classes_per_set,
                                         num_samples_per_class=samples_per_class,
                                         nClasses = 0, image_size = 28, test_only=test_only).to(device)
        # self.matchingNet = MatchingNetwork(batch_size=batch_size, k_bits=k_bits,
        #                                    keep_prob=self.keep_prob, num_channels=channels,
        #                                    fce=fce,
        #                                    num_classes_per_set=classes_per_set,
        #                                    num_samples_per_class=samples_per_class,
        #                                    nClasses=0, image_size=28, test_only=test_only)
        self.optimizer = 'adam'
        self.lr = 1e-03
    ...
    def run_training_epoch(self, total_train_batches):
        ...
        with tqdm.tqdm(total=total_train_batches) as pbar:
            for i in range(total_train_batches):  # train epoch
                ...
                if self.isCudaAvailable:
                    acc, c_loss_value = self.matchingNet(x_support_set.to(device), y_support_set_one_hot.to(device),
                ...
                iter_out = "tr_loss: {}, tr_accuracy: {}".format(c_loss_value.data.item(), acc.data.item())
    ...
    def run_validation_epoch(self, total_val_batches):
        ...
        with tqdm.tqdm(total=total_val_batches) as pbar:
            for i in range(total_val_batches):  # validation epoch
                ...
                iter_out = "val_loss: {}, val_accuracy: {}".format(c_loss_value.data.item(), acc.data.item())
                pbar.set_description(iter_out)
                pbar.update(1)
    ...
    def run_testing_epoch(self, total_test_batches):
        ...
        with tqdm.tqdm(total=total_test_batches) as pbar:
            for i in range(total_test_batches):
                ...
                iter_out = "test_loss: {}, test_accuracy: {}".format(c_loss_value.data.item(), acc.data.item())
    ...
    def save_model(self):
        state = {
            'state_dict': self.matchingNet.state_dict()
        }
        utils.utility.save_checkpoint(state, checkpoint=self.LOG_DIR + '/model')
        return state

    def load_model(self):
        ckpt = torch.load(self.SAVE_DIR + '/model/model_best.pth.tar')
        checkpoint = ckpt['state_dict'] if 'state_dict' in ckpt else ckpt
        self.matchingNet.load_state_dict(checkpoint)
